...
export class ReviewPage {
  ...
  async hasReviewIdInUrl() {
    await expect(this.page).toHaveURL(
      /^http:\/\/localhost:3000\/review\/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/,
    );
  }
  ...
  async getRecentAssistantMessage() {
    const content = await this.page.getByTestId('review-content').innerText();
    return {
      content,
    };
  }
